...
# --- Helper function to extract headlines from HTML ---
def extract_headlines(html: str) -> list:
    import re
    # For simplicity, try to extract text inside header tags (h1-h3)
    pattern = r'<h[1-3][^>]*>(.*?)</h[1-3]>'
    headlines = re.findall(pattern, html, re.IGNORECASE | re.DOTALL)
    clean_headlines = []
    for hl in headlines:
        hl_clean = re.sub(r'<[^>]+>', '', hl)  # strip html tags
...
async def run_task(instructions: str):
    """
    Initializes resources, creates agent, runs the task, and if no final result is present,
    attempts to extract headlines from the currently open page before cleanup.
    Returns a JSON object with the final result.
    """
    llm_instance = None
    ...
    try:
        ...
        # 3. Initialize Context – create a browser context and do not let the Agent close it immediately.
        ...
        # 4. Initialize Agent with the injected browser_context so that it persists for headline extraction.
        ...
        # 6. Fallback extraction: if the result text is empty or a default placeholder, attempt to extract headlines.
        ...
        # 7. Cleanup – attempt to close context and browser (warnings about forced closure are benign)
